...
from helper_funcs import send_mail, convert_to_coreML
...
class Gen_Model():
	...
	def write(self, game, version):
		model_path = run_folder + 'models/version' + "{0:0>4}".format(version)
		self.model.save(model_path + '.h5')
		os.system('cp ' + model_path + '.h5 /tmp/version' + "{0:0>4}".format(version) + '.h5')
		convert_to_coreML(model_path + '.h5', "{0:0>4}".format(version))
		send_mail("connect4bot@gmail.com", "Version " + "{0:0>4}".format(version), "Check out the latest version!", [config.GMAIL_ACCOUNT], [ run_folder + 'models/connect4_model' + "{0:0>4}".format(version) + '.mlmodel'])
